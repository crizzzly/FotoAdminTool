package FileBrowser.controller;

import FileBrowser.model.Foto;
    ...
    private static Foto moveFile(Path path, Foto foto) {
        ...
        if (!(new File(path + "/" + file.getFileName()).exists())) {
            try {
                ...
                LOGGER.severe("Failed to move file"+file.getFileName()+" to "+ target);
            }
        }
    ...
    private static void createNewSubDir(String newSubDir) {
        ...
        if (!Files.exists(newDir)) {
            try {
                ...
                LOGGER.severe("Failed to create directory"+ newDir);
            }
        }
        subDirs.add(newDir);
    }
